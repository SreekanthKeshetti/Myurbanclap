...
const ServiceCard = ({ service, onBook }) => {
  ...
  return (
    <div className="uc-service-card w-100 h-100 d-flex flex-column" onClick={goToDetails} style={{ maxWidth: "100%" }}>
      {/* IMAGE AREA */}
      <div className="img-container p-2 position-relative">
        ...
        <img src={service.image || "https://via.placeholder.com/300"} alt={service.name} style={{ height: "200px" }} />
      </div>

      <Card.Body className="p-3 pt-2 d-flex flex-column flex-grow-1 border-0 bg-white">
        {/* TITLE & RATING */}
        <div className="d-flex justify-content-between align-items-start mb-2">
          <h5 className="fw-bold mb-0 text-dark text-truncate pe-2" style={{ fontSize: "1.1rem" }}>
            {service.name}
          </h5>
          <div className="d-flex align-items-center small bg-light px-2 py-1 rounded border">
            ...
            <span className="fw-bold text-dark">{service.rating > 0 ? service.rating.toFixed(1) : "4.8"}</span>
          </div>
        </div>

        <p className="text-muted small mb-3 flex-grow-1" style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", lineHeight: "1.5" }}>
          {service.description}
        </p>
        ...
        <div className="d-flex justify-content-between align-items-center mt-2">
          <div>
            ...
            <small className="text-muted d-block" style={{ fontSize: "10px" }}>Starts at</small>
          </div>
